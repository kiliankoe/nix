...
cubesat runs pangolin, kanidm, and uptime-kuma under `hosts/cubesat/services/`. Pangolin resources are declaratively managed: newt on kepler (nix-private) applies a blueprint generated from the private service map, and the "home" newt site's WireGuard tunnel carries the traffic (see the nix-private README for the blueprint semantics and the role architecture). A few resources stay hand-made in the dashboard for documented reasons; treat the dashboard as read-only for everything else — edits there are overwritten by the nightly blueprint re-apply. Kanidm (`auth.kilko.de`) is the IdP behind pangolin's SSO wall and the in-app logins of grafana and jellyfin; persons/groups are provisioned authoritatively from nix (public core in `kanidm.nix`, friends and private clients in nix-private). Not visible from the config: the enterprise license is activated in the dashboard, the K-ID identity provider + role mappings live in pangolin's DB, and the Authentication/Admin Action log pages are gated by per-org Log Retention settings (Org Settings → General → Security; retention 0 = logging disabled).

Version coupling to watch on flake bumps: `fosrl-pangolin` (unpinned) and `fosrl-newt` move in loose lockstep — after a pangolin bump, check `journalctl -u newt` on kepler for protocol grumbles; kanidm upgrades must never skip a minor (`kanidm_1_11` pin in `kanidm.nix`).
