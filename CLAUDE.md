...
- `helix.nix` formats js/jsx/ts/tsx/json/jsonc with biome through the `hx-biome-format` wrapper (`scripts/hx-biome-format.sh`), and markdown with prettier because biome doesn't support it. The same six languages also get `biome lsp-proxy` as a second language server. The wrapper exists to impose a house style without overriding projects that decided otherwise. Details worth not re-deriving:
  ...
  - Format-on-save is off; `:fmt` is explicit. helix's `auto-format` defaults false and is deliberately not set, because `editor.auto-save.focus-lost` is on and the two together reformat on every focus change. It can be turned on per project with a `.helix/languages.toml` in the workspace root holding `[[language]]` / `name = "tsx"` / `auto-format = true` — verified working, and it merges over this config rather than replacing the formatter.
